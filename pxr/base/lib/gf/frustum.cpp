                                ...
                                double windowWidth, double windowHeight)
{
    ...
    int clipFlags = 0;
    for (int i = 0; i < 8; ++i) {
        GfVec4d const &clipPos = points[i];
                                ...
                                float windowWidth, float windowHeight)
{
    ...
    const GfVec3f &localMax = bbox.GetRange().GetMax();
    const GfMatrix4f &matrix = viewProjMat;
    
    points[0] = GfVec4f(localMin[0], localMin[1], localMin[2], 1);
    points[1] = GfVec4f(localMax[0], localMax[1], localMax[2], 1);

    // Transform min/max bbox local space points into clip space
    points[0] = points[0] * matrix;
    points[1] = points[0] * matrix;
    
    // Small prim cull
    GfVec2f screenSpace[2];
    ...
    inv = 1.0f / points[7][3];
    screenSpace[1][0] = points[1][0] * inv;
    screenSpace[1][1] = points[1][1] * inv;
    
    float const threshold = 4.0f; // number of pixels in a dimension
    float dx = (screenSpace[1][0] - screenSpace[0][0]) * windowWidth;
    float dy = (screenSpace[1][1] - screenSpace[0][1]) * windowHeight;
    ...
    // Compute the remaining 8 points of the bbox in
    // bbox local space.
    points[2] = GfVec4f(localMin[0], localMin[1], localMax[2], 1);
    points[3] = GfVec4f(localMin[0], localMax[1], localMin[2], 1);
    points[4] = GfVec4f(localMin[0], localMax[1], localMax[2], 1);
    points[5] = GfVec4f(localMax[0], localMin[1], localMin[2], 1);
    points[6] = GfVec4f(localMax[0], localMin[1], localMax[2], 1);
    points[7] = GfVec4f(localMax[0], localMax[1], localMin[2], 1);
    ...
    int clipFlags = 0;
    for (int i = 0; i < 8; ++i) {
        GfVec4f const &clipPos = points[i];
