...
///
/// A \c GfBBox3f is more useful than using just \c GfRange3d instances
...
/// root, a \c GfBBox3f is generally much smaller than the \c GfRange3d
...
///
/// To distinguish when this loosening is necessary, each \c GfBBox3f
...
class GfBBox3f {
  ...
  public:
    ...
    GfBBox3f(const GfRange3f &box, const GfMatrix4f &matrix) {
        Set(box, matrix);
        _hasZeroAreaPrimitives = false;
    }

    /// Sets the axis-aligned box and transformation matrix.
    void                Set(const GfRange3f &box, const GfMatrix4f &matrix) {
        _box = box;
        _SetMatrices(matrix);
    }
    ...
    const GfMatrix4f &  GetInverseMatrix() const {
        return _inverse;
    }
    ...
    void                Transform(const GfMatrix4f &matrix) {
    ...
    /// aligning the result.
    GF_API
    GfRange3f           ComputeAlignedRange() const;
    ...
    GfRange3f           ComputeAlignedBox() const {
    ...
    /// two resulting boxes.
    GF_API
    static GfBBox3f     Combine(const GfBBox3f &b1, const GfBBox3f &b2);

    /// Returns the centroid of the bounding box.
    /// The centroid is computed as the transformed centroid of the range.
    GF_API
    GfVec3f             ComputeCentroid() const;

    /// Hash.
    friend inline size_t hash_value(const GfBBox3f &b) {
        size_t h = 0;
    ...
    bool                operator ==(const GfBBox3f &b) const {
        return (_box    == b._box &&
                _matrix == b._matrix);
    }
    ...
    /// equal. (To compare equality of the actual boxes, you can compute both
    /// aligned boxes and test the results for equality.)
    bool                operator !=(const GfBBox3f &that) const {
        return !(*this == that);
    }
  ...
  private:
    ...
    /// degeneracies.
    GF_API
    void                _SetMatrices(const GfMatrix4f &matrix);
    ...
    static GfBBox3f     _CombineInOrder(const GfBBox3f &b1, const GfBBox3f &b2);
};

/// Output a GfBBox3f using the format [(range) matrix zeroArea]
///
/// The zeroArea flag is true or false and indicates whether the
/// bbox has zero area primitives in it.
///
/// \ingroup group_gf_DebuggingOutput
GF_API std::ostream& operator<<(std::ostream&, const GfBBox3f&);
