...
bool
UsdImagingMetalDrawModeAdapter::_IsMaterialPath(SdfPath const& path)
{
    return IsChildPath(path) && path.GetNameToken() == _tokens->material;
}

bool
UsdImagingMetalDrawModeAdapter::_IsTexturePath(SdfPath const& path)
{
    if (!IsChildPath(path)) {
        return false;
    }
...
std::string
UsdImagingMetalDrawModeAdapter::_GetSurfaceShaderSource()
{
    boost::scoped_ptr<GLSLFX> gfx(GLSLFX::New(UsdImagingMetalPackageDrawModeShader()));
    if (!gfx->IsValid()) {
...
GfRange3d
UsdImagingMetalDrawModeAdapter::_ComputeExtent(UsdPrim const& prim)
{
    HD_TRACE_FUNCTION();
