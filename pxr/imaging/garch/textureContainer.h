...
//
#ifndef PXR_IMAGING_GARCH_TEXTURE_CONTAINER_H
#define PXR_IMAGING_GARCH_TEXTURE_CONTAINER_H

/// \file garch/textureContainer.h
...
TF_DECLARE_REF_PTRS(GarchTexture);

/// \class GarchTextureContainer
///
...
class GarchTextureContainer : public GarchTexture {
public:
   
    /// Get texture handle for a particular frame, subimage, grid, ...
    GARCH_API
    GarchTextureHandlePtr GetTextureHandle(Identifier const &identifier);

    /// Implements the garbage collection of textures in this container.
    ///
    /// When the Garch clients give up all their references to all textures in
...
protected:
    ...
    virtual GarchTextureRefPtr _CreateTexture(Identifier const &identifier) = 0;

    // Texture handles for frames, subimage, grids, ...
    std::map<Identifier, GarchTextureHandleRefPtr> _textureHandles;
};
