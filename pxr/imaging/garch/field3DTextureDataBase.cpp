...
static
GarchField3DTextureDataFactoryBase *
_GetFactory()
{
    const TfType baseType = TfType::Find<GarchField3DTextureDataBase>();
    if (!baseType) {
    ...
    GarchField3DTextureDataFactoryBase * const factory =
        pluginType.GetFactory<GarchField3DTextureDataFactoryBase>();
    if (!factory) {
...
GarchField3DTextureDataBaseRefPtr
...
{
    static GarchField3DTextureDataFactoryBase * const factory = _GetFactory();
    if (!factory) {
        return TfNullPtr;
    }
    return factory->_New(
