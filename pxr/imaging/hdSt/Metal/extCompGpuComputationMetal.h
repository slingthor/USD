...
//
#ifndef HDST_EXT_COMP_GPU_COMPUTATION_METAL_H
#define HDST_EXT_COMP_GPU_COMPUTATION_METAL_H
...
class HdStExtCompGpuComputationMetal final : public HdStExtCompGpuComputation {
public:
    ...
    HdStExtCompGpuComputationMetal(
            SdfPath const &id,
    ...
    CreateGpuComputationMetal(
        HdSceneDelegate *sceneDelegate,
        HdExtComputation const *sourceComp,
        HdExtComputationPrimvarDescriptorVector const &compPrimvars);

    HDST_API
    virtual ~HdStExtCompGpuComputationMetal() = default;

private:
    HDST_API
    virtual void _Execute(HdStGLSLProgramSharedPtr const &computeProgram,
                          std::vector<int32_t> const &_uniforms,
                          HdStBufferArrayRangeSharedPtr outputBar) override;

    HdStExtCompGpuComputationMetal()                                   = delete;
    HdStExtCompGpuComputationMetal(const HdStExtCompGpuComputation &)  = delete;
    HdStExtCompGpuComputationMetal &operator =
        (const HdStExtCompGpuComputationMetal &) = delete;
};

PXR_NAMESPACE_CLOSE_SCOPE

#endif // HDST_EXT_COMP_GPU_COMPUTATION_METAL_H
