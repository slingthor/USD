...
HdSt_CodeGenGLSL::HdSt_CodeGenGLSL(HdStShaderCodeSharedPtrVector const &shaders)
    : _geometricShader(), _shaders(shaders)
{
}

HdSt_CodeGenGLSL::ID
HdSt_CodeGenGLSL::ComputeHash() const
{
    HD_TRACE_FUNCTION();
...
HdSt_CodeGenGLSL::Compile()
{
    ...
    if (HdVtBufferSource::GetDefaultMatrixType() == HdTypeFloatMat4) {
...
void
HdSt_CodeGenGLSL::_GenerateDrawingCoord()
{
...
void
HdSt_CodeGenGLSL::_GenerateConstantPrimvar()
{
    /*
...
void
HdSt_CodeGenGLSL::_GenerateInstancePrimvar()
{
    /*
...
void
HdSt_CodeGenGLSL::_GenerateElementPrimvar()
{
    /*
...
void
HdSt_CodeGenGLSL::_GenerateShaderParameters()
{
    /*
      ------------- Declarations -------------
    ...
    TF_FOR_ALL (it, _metaData.shaderParameterBinding) {
        ...
        if (bindingType == HdBinding::FALLBACK) {
            ...
            accessors
                ...
                << "  int shaderCoord = GetDrawingCoord().shaderCoord;\n"
                << "  uvec2 samplerHandle = shaderData[shaderCoord]."
                << it->second.name << ";\n"
                << "  " << _GetUnpackedType(it->second.dataType, false)
                << " result = "
                << _GetPackedTypeAccessor(it->second.dataType, false)
                << "(texture(sampler2D(samplerHandle), coord)" << swizzle
                << ");";

            if (it->second.processTextureFallbackValue) {
                // Check whether texture is valid (using bindless handle)
                //
            ...
            if (caps.bindlessTextureEnabled) {
                accessors
                    ...
                    << "HdGetSampler_" << it->second.name << "() {\n"
                    << "  int shaderCoord = GetDrawingCoord().shaderCoord; \n"
                    << "  return sampler3D(shaderData[shaderCoord]."
                    << it->second.name << ");\n"
                    << "}\n";
            }
            ...
            accessors
                ...
                << " result = "
                << _GetPackedTypeAccessor(it->second.dataType, false)
                << "(texture(sampler3D(samplerHandle), sampleCoord)"
                << swizzle << ");\n";
...
void
HdSt_CodeGenGLSL::_GenerateTopologyVisibilityParameters()
{
    std::stringstream declarations;
    std::stringstream accessors;
    TF_FOR_ALL (it, _metaData.topologyVisibilityData) {
