...
{
    GarchSimpleLight light = GarchSimpleLight();
    if (pathIdx < _lightIds.size()) {
        light = _delegate.GetParameter<GarchSimpleLight>(
...
HdxTaskController::_ReplaceLightSprim(size_t const& pathIdx, 
                        GarchSimpleLight const& light, SdfPath const& pathName)
{                
...
HdxTaskController::SetLightingState(GarchSimpleLightingContextPtr const& src)
{
    ...
    if (_lightIds.size() < lights.size()) {
        ...
        for (size_t i = 0; i < lights.size(); ++i) {
            ...
            GarchSimpleLight currLight = _GetLightAtId(i);
    ...
    for (size_t i = 0; i < lights.size(); ++i) {
        GarchSimpleLight light = _GetLightAtId(i);
        if (light != lights[i]) {
