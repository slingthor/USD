...
HioGlslfx::_GetSourceKeys(const TfToken &shaderStageKey) const
{
    if (!_config) {
        return HioGlslfxConfig::SourceKeys();
    }
    
    return _config->GetSourceKeys(shaderStageKey);
}
...
HioGlslfxConfig::SourceKeys
HioGlslfx::GetSourceKeys(const TfToken &shaderStageKey) const
{
    return _GetSourceKeys(shaderStageKey);
}
