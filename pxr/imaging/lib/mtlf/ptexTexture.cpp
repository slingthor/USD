...
#include "pxr/imaging/mtlf/glContext.h"
...
{
    typedef MtlfPtexTexture Type;
    TfType t = TfType::Define<Type, TfType::Bases<GarchTexture> >();
    t.SetFactory< GarchTextureFactory<Type> >();
}

//------------------------------------------------------------------------------
MtlfPtexTextureRefPtr
MtlfPtexTexture::New(const TfToken &imageFilePath)
{
...
MtlfPtexTexture::GetBindings(TfToken const & identifier, GLuint samplerName) const
{
...
bool
MtlfPtexTexture::IsMinFilterSupported(GLenum filter)
{
    switch(filter) {
    case GL_NEAREST:
    case GL_LINEAR:
...
bool
MtlfPtexTexture::IsMagFilterSupported(GLenum filter)
{
    switch(filter) {
    case GL_NEAREST:
    case GL_LINEAR:
