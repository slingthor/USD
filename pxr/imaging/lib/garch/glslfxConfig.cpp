...
VtDictionary Garch_GetDictionaryFromInput(const string &input, const string &filename, string *errorStr);

GLSLFXConfig *
GLSLFXConfig::Read(string const & input, string const & filename, string *errorStr)
{
    return new GLSLFXConfig(
        Garch_GetDictionaryFromInput(input, filename, errorStr), errorStr );
}

GLSLFXConfig::GLSLFXConfig(VtDictionary const & dict, string * errors)
{
    _Init(dict, errors);
}

void
GLSLFXConfig::_Init(VtDictionary const & dict, string * errors)
{
...
GLSLFXConfig::SourceKeys
GLSLFXConfig::GetSourceKeys(TfToken const & shaderStageKey) const
{
    GLSLFXConfig::SourceKeys ret;
    TfMapLookup(_sourceKeyMap, shaderStageKey, &ret);
    return ret;
}
 
GLSLFXConfig::_SourceKeyMap
GLSLFXConfig::_GetSourceKeyMap(VtDictionary const & dict,
...
static GLSLFXConfig::Role
_GetRoleFromString(string const & roleString, string *errorStr)
{
    if (roleString == _tokens->color) {
        return GLSLFXConfig::RoleColor;
    }

    *errorStr = TfStringPrintf("Unknown role specification: %s",
                               roleString.c_str());
    return GLSLFXConfig::RoleNone;
}


GLSLFXConfig::Parameters
GLSLFXConfig::GetParameters() const
{
    return _params;
}

GLSLFXConfig::Parameters
GLSLFXConfig::_GetParameters(VtDictionary const & dict,
                                 string *errorStr) const
{
    Parameters ret;
    ...
    for (std::string const& paramName : paramOrder) {
        ...
        TF_DEBUG(GARCH_DEBUG_GLSLFX).Msg("        param: %s\n",
            paramName.c_str());
...
GLSLFXConfig::GetTextures() const
{
    return _textures;
}

GLSLFXConfig::Textures
GLSLFXConfig::_GetTextures(VtDictionary const & dict,
                               string *errorStr) const
{
    Textures ret;
    ...
    for (const std::pair<std::string, VtValue>& p : texturesDict) {
        ...
        TF_DEBUG(GARCH_DEBUG_GLSLFX).Msg("        texture: %s\n",
            textureName.c_str());
...
GLSLFXConfig::GetAttributes() const
{
    return _attributes;
}

GLSLFXConfig::Attributes
GLSLFXConfig::_GetAttributes(VtDictionary const & dict,
                                 string *errorStr) const
{
    Attributes ret;
    ...
    for (const std::pair<std::string, VtValue>& p : attributesDict) {
        ...
        TF_DEBUG(GARCH_DEBUG_GLSLFX).Msg("        attribute: %s\n",
            attributeName.c_str());
...
GLSLFXConfig::GetMetadata() const
{
    return _metadata;
}

GLSLFXConfig::MetadataDictionary
GLSLFXConfig::_GetMetadata(VtDictionary const & dict,
                              string *errorStr) const
{
