...
TF_DEFINE_ENV_SETTING(GARCH_IMAGE_PLUGIN_RESTRICTION, "",
                  "Restricts GarchImage plugin loading to the specified plugin");

TF_INSTANTIATE_SINGLETON(GarchImageRegistry);

GarchImageRegistry&
GarchImageRegistry::GetInstance()
{
    return TfSingleton<GarchImageRegistry>::GetInstance();
}

GarchImageRegistry::GarchImageRegistry() :
    _typeMap(new GarchRankedTypeMap)
{
    // Register all image types using plugin metadata.
    _typeMap->Add(TfType::Find<GarchImage>(), "imageTypes",
                 GARCH_DEBUG_TEXTURE_IMAGE_PLUGINS,
                 TfGetEnvSetting(GARCH_IMAGE_PLUGIN_RESTRICTION));
}

GarchImageSharedPtr
GarchImageRegistry::_ConstructImage(std::string const & filename)
{
...
bool
GarchImageRegistry::IsSupportedImageFile(std::string const & filename)
{
