    ...
    bool isIndexed)
{
    ...
    id<MTLFunction> function = _functions[type];
    
    if (!function) {
        function = [_library newFunctionWithName:_functionNames[type]];
        function.label = _functionNames[type];
        _functions[type] = function;
    }
    
    if (!_argumentEncoder) {
        _argumentEncoder =
            [function newArgumentEncoderWithBufferIndex:HgiMetalArgumentIndexICB];
    }
...
// XXX: the encodedLength doesn't change, so it doesn't need to be a param.
id<MTLBuffer>
HgiMetalIndirectCommandEncoder::_AllocateArgumentBuffer()
{
    ...
    if (!buffer) {
        buffer =
            [_device newBufferWithLength:_argumentEncoder.encodedLength
                                 options:_bufferStorageMode];
    }
    ...
    uint32_t patchBaseVertexByteOffset)
{
    ...
    id<MTLFunction> function = _GetFunction(pipelineDesc, !!indexBuffer);
    ...
    std::unique_ptr<HgiMetalIndirectCommands> commands =
        std::make_unique<HgiMetalIndirectCommands>(
            ...
            _AllocateArgumentBuffer(),
            _hgi->GetArgBuffer());

    [_argumentEncoder setArgumentBuffer:commands->indirectArgumentBuffer
                                 offset:0];
    ...
    [_argumentEncoder setIndirectCommandBuffer:commands->indirectCommandBuffer
    ...
    [_argumentEncoder setBuffer:commands->mainArgumentBuffer
                         offset:0
    ...
    _SetArg(_argumentEncoder, ArgIndex_PrimitiveType, mtlPrimitiveType);
    _SetArg(_argumentEncoder, ArgIndex_DrawStepFunction, drawCommandStep);
    _SetArg(_argumentEncoder, ArgIndex_NumControlPoints, controlPointCount);
    _SetArg(_argumentEncoder, ArgIndex_PatchBaseVertexByteOffset,
        patchBaseVertexByteOffset);
    _SetArg(_argumentEncoder, ArgIndex_NumBuffers, bindings.size());
    _SetArg(_argumentEncoder, ArgIndex_NumPatchStepFuncs,
        (uint32_t)stepFunctions.GetPatchBaseDescs().size());
    HgiMetalStepFunctionDesc* argPatchStepDescs = (HgiMetalStepFunctionDesc*)
        [_argumentEncoder constantDataAtIndex:ArgIndex_PatchStepFunctions];
    ...
    if (indexBuffer) {
        ...
        [_argumentEncoder setBuffer:mtlIndexBuffer->GetBufferId()
                             offset:0
    ...
    for (auto const& binding : bindings) {
        ...
        [_argumentEncoder setBuffer:mtlBuffer->GetBufferId()
    ...
    NSError *error;
    id<MTLComputePipelineState> icbPipelineState =
        [_device newComputePipelineStateWithFunction:function
                                               error:&error];
