...
void
HdxFullscreenShaderGL::SetProgramToCompositor(bool depthAware) {
...
void
HdxFullscreenShaderGL::SetUniform(TfToken const& name, VtValue const& data)
{
    if (data.IsEmpty()) {
...
void
HdxFullscreenShaderGL::_CreateBufferResources()
{
...
void
HdxFullscreenShaderGL::_CreateTextureResources(GLuint *texture)
{
...
void
HdxFullscreenShaderGL::SetTexture(TfToken const& name, int width, int height,
...
void
HdxFullscreenShaderGL::_SetUniform(GLuint programId,
...
void 
HdxFullscreenShaderGL::Draw(TextureMap const& textures)
{
    // No-op if we haven't set a shader.
    if (!_program) {
        if (_glslfx.IsEmpty() || _technique.IsEmpty()) {
            TF_CODING_ERROR("HdxFullscreenShaderGL: caller needs to set a program "
                            "before calling draw!");
        }
        return;
    }
    ...
    // A note here: HdxFullscreenShaderGL is used for all of our plugins and has to be
    ...
    HdStGLSLProgram *glslProgram =
        dynamic_cast<HdStGLSLProgram*>(_program.get());
